...
class GroupScaling1D(torch.nn.Module):
...
class MaskPowerNorm(torch.nn.Module):
    """
    ...
    def __init__(self, num_features, eps=1e-5, alpha_fwd=0.9, alpha_bkw=0.9,
        ...
        self.register_parameter('weight', torch.nn.Parameter(torch.ones(num_features)))
    ...
    def forward(self, input, pad_mask=None, is_encoder=False):
        ...
        input = input.reshape(input.num_kernels(0), self.num_features, -1)
